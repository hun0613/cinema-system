  ...
  return (
    <ModalAtom onCloseModal={onCloseModal}>
      <div className="flex h-fit w-full flex-col items-start justify-start p-5 tablet:p-10">
        {/* title */}
        <h1 className="h-fit w-fit p-2 text-left font-NMSNeo3 text-lg text-borderColor tablet:text-xl">{data.title}</h1>
        <h1 className="h-fit w-full p-2 text-left font-NMSNeo3 text-base text-borderColor tablet:text-lg">줄거리</h1>
        {/* 줄거리 */}
        <p className="h-fit w-full p-2 text-left font-NMSNeo2 text-sm text-borderColor mobile:text-base">{data.summary}</p>
